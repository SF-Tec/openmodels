...
def test_feature_hasher_serialization():
    ...
    assert not np.all(transformed_dict == 0), "Transformation failed for dictionary input"
...
def test_patch_extractor_with_serialization(patch_size=(10, 10), max_patches=None):
    """
    Helper function to test PatchExtractor with serialization and deserialization.

    Parameters
    ----------
    patch_size : tuple of int, optional
        The dimensions of one patch (default is (10, 10)).
    max_patches : int or float, optional
        The maximum number of patches per image to extract. If None, extract all patches.

    Returns
    -------
    dict
        A dictionary containing the original image shape, patches shape, and reconstructed image shape.
    """
   # Ensure patch_size is a tuple
    ...
    X = load_sample_images().images[1]  # Use the second image in the dataset
    X = X[None, ...]  # Add a batch dimension

    # Initialize PatchExtractor
    pe = image.PatchExtractor(patch_size=patch_size, max_patches=max_patches)

    # Serialize the PatchExtractor
    manager = SerializationManager(SklearnSerializer())
    serialized_pe = manager.serialize(pe)

    # Deserialize the PatchExtractor
    deserialized_pe = manager.deserialize(serialized_pe)

    # Extract patches using the deserialized PatchExtractor
    patches = deserialized_pe.transform(X)

    # Reconstruct the image from patches
    reconstructed_image = image.reconstruct_from_patches_2d(patches, X.shape[1:])

    # Validate the reconstruction
    assert np.allclose(X[0], reconstructed_image, atol=1e-5), "Reconstructed image does not match the original!"

    return {
        "original_shape": X.shape,
        "patches_shape": patches.shape,
        "reconstructed_shape": reconstructed_image.shape,
    }